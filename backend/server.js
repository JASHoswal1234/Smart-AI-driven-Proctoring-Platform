...
import { uploadScreenshot, uploadQuestionImage, uploadQuestionAudio } from "./utils/cloudinaryUpload.js";
...
app.post("/api/upload/question-audio", async (req, res) => {
  try {
    const { dataUrl, examId } = req.body;
    if (!dataUrl || !examId) {
      return res.status(400).json({ message: "Missing required fields" });
    }
    const url = await uploadQuestionAudio(dataUrl, examId);
    res.json({ secure_url: url });
  } catch (err) {
    console.error("Question audio upload error:", err.message);
    res.status(500).json({ message: "Upload failed" });
  }
});
